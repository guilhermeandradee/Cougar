...
class ProblemaService {
    async cadastrarProblema(categoria, descricao, resolucao) {
        ...
        try {
            ...
            const descricao_palavras_chave = await gerar_palavras_chave(descricao); // Para isso funcionar, atualizar API key
            const newProblem = new Problema({ id, categoria, descricao, resolucao, descricao_palavras_chave });
           
            savedProblem = await newProblem.save();
            console.log('Problema salvo:', savedProblem);
        } catch (error) {
            console.error('Erro ao cadastrar problema:', error);
            throw new Error('Não foi possível cadastrar o problema.');
        } finally {
    ...
    async atualizarProblema(id, newData) {
        ...
        try { 
            ...
            throw new Error('Não foi possível atualizar o problema.');
        } finally {
            await closeConnectionDB();
        }
    }
    ...
    async getOneProblem(id) {
        ...
        try {
            const problemaEncontrado = await Problema.findOne({ id });
            return problemaEncontrado;
        } catch (error) {
